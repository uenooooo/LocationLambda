...
internal class GeofenceEventProcessor(
    ...
    fun process(event: GeofencingEvent) {
        ...
        val updatedRules =
                rules.map { rule ->
                    ...
                    hasUpdates = true
                    rule.copy(lastTriggeredAt = now, lastTriggeredTransition = transition).also {
                            triggeredRule ->
                        val triggeredRuleUi =
                                triggeredRule
                                        .toUi()
                                        .copy(transitions = listOf(transition.toTransitionUi()))
                        val distanceMeters =
                                distanceMeters(
                                        currentLat = event.triggeringLocation?.latitude,
                                        currentLng = event.triggeringLocation?.longitude,
                                        targetLat = rule.latitude,
                                        targetLng = rule.longitude
                                )

                        if (distanceMeters != null && distanceMeters > 1000f) {
                            logIgnored(
                                    rule.name,
                                    "現在地が登録地点から1km以上離れている distance=${distanceMeters.toInt()}m"
                            )
                            statusRepository.markIgnored(rule.name, "1km以上離れている")
                            return@also
                        }
    ...
    private fun logIgnored(title: String, detail: String) {
        debugLogRepository.append(type = DebugLogType.IGNORED, title = title, detail = detail)
    }
    ...
    private fun LocationRule.matchesTransition(googleTransition: Int): Boolean {
        return when (googleTransition) {
            Geofence.GEOFENCE_TRANSITION_ENTER -> LocationTransition.includesEnter(transitionType)
            Geofence.GEOFENCE_TRANSITION_EXIT -> LocationTransition.includesExit(transitionType)
            else -> false
        }
    }
    ...
    private fun LocationRule.isBoundaryJitter(googleTransition: Int, now: Long): Boolean {
